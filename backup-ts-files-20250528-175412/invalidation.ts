import { invalidateCache, requestManager } from "./cache";
...
const CACHE_DEPENDENCY_GROUPS: CacheDependencyGroup[] = [
  ...
  },
  {
    ...
    keys: [CacheKeyPrefix.ServicesDetail],
    dependsOn: [CacheGroup.Services],
    priority: "medium",
  },
  {
    name: CacheGroup.FeaturedServices,
    keys: [CacheKeyPrefix.ServicesFeatured],
    dependsOn: [CacheGroup.Services],
    priority: "high",
  },
  ...
  // Dashboard Client
  {
    ...
    keys: [CacheKeyPrefix.ClientStats],
    priority: "high",
  },
  {
    name: CacheGroup.ClientActivities,
    keys: [CacheKeyPrefix.ClientActivities],
    priority: "medium",
  },
  {
  ...
  // Dashboard Freelance
  {
    ...
    keys: [CacheKeyPrefix.FreelanceStats],
    priority: "high",
  },
  {
    name: CacheGroup.FreelanceActivities,
    keys: [CacheKeyPrefix.FreelanceActivities],
    priority: "medium",
  },
  {
    ...
    dependsOn: [
      CacheGroup.FreelanceStats,
      CacheGroup.FreelanceActivities,
      CacheGroup.Services,
    ],
    priority: "high",
  },
  ...
  // Taxonomie
  {
    ...
    keys: [CacheKeyPrefix.Categories, CacheKeyPrefix.Subcategories],
    priority: "medium",
  },
];
...
export const makeCacheKey = (
  ...
  const sortedParams = Object.keys(params)
    ...
    .reduce((result: Record<string, any>, key: string) => {
      ...
      if (
        params[key as keyof CacheParams] !== undefined &&
        params[key as keyof CacheParams] !== null
      ) {
...
class CacheInvalidationManager {
  ...
  public invalidateKey(
    ...
    if (includePrefix) {
      this.groups.forEach((group, groupName) => {
        const matchesPrefix = group.keys.some(
          (groupKey) =>
            key.toString().startsWith(groupKey) ||
            groupKey.startsWith(key.toString()),
        );
    ...
    if (typeof window !== "undefined") {
      window.dispatchEvent(
        new CustomEvent("vynal:cache-invalidated", {
  ...
  private processInvalidations(): number {
    ...
    const processGroup = (groupName: string) => {
      ...
      this.groups.forEach((dependentGroup, dependentName) => {
        if (
          dependentGroup.dependsOn &&
          dependentGroup.dependsOn.includes(groupName)
        ) {
    ...
    if (totalInvalidated > 0 && typeof window !== "undefined") {
      window.dispatchEvent(
        new CustomEvent("vynal:cache-bulk-invalidated", {
          detail: {
            count: totalInvalidated,
            groups: Array.from(processedGroups),
          },
        }),
      );
    }
  ...
  public invalidateByPrefix(prefix: CacheKeyPrefix | string): number {
    ...
    this.groups.forEach((group, groupName) => {
      const matchesPrefix = group.keys.some((key) =>
        key.startsWith(prefix.toString()),
      );
      if (matchesPrefix) {
        this.pendingInvalidations.add(groupName);
        count++;
