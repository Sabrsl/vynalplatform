...
export class SystemStatusService {
    ...
    try {
      if (isServer) {
        ...
        try {
          ...
          console.error(
            "Erreur d'écriture, données conservées uniquement en cache",
          );
        }
      } else {
        // Côté client, on ne peut pas écrire directement dans le fichier
        console.warn(
          "Tentative d'écriture côté client. Utilisez les API endpoints.",
        );
      }
    } catch (error) {
  ...
  static async updateFeatureStatus(
    ...
    try {
      ...
      const featureIndex = this.cache.features.findIndex(
        (f) => f.id === featureId,
      );
      if (featureIndex === -1) {
   ...
   */
  static async getIncidentsByFeatureId(
    featureId: string,
    ...
    try {
      ...
      const incidents = this.cache.incidents.filter(
      ...
      console.error(
        "Erreur lors de la récupération des incidents par fonctionnalité",
      );
      throw error;
    }
  }
  ...
  static async createIncident(incidentData: {
    ...
    try {
      ...
      const featureIndex = this.cache.features.findIndex(
        (f) => f.id === incidentData.feature_id,
      );
      if (featureIndex !== -1) {
        this.cache.features[featureIndex] = {
          ...this.cache.features[featureIndex],
  ...
  static async addIncidentUpdate(
    ...
    try {
      ...
      // Trouver l'incident
      const incidentIndex = this.cache.incidents.findIndex(
        (i) => i.id === incidentId,
      );
      if (incidentIndex === -1) {
      ...
      const updatedIncident: SystemIncident = {
        ...incident,
        status: updateData.status,
        resolved_at: updateData.resolve
