...
function setImageSrcSafely(image: HTMLImageElement, url: string): void {
  ...
  if (isSecureBlobUrl(url)) {
    // Utiliser l'API URL pour créer et valider l'URL avant de l'attribuer
    try {
      // Création d'un nouvel objet URL pour valider la syntaxe
      const safeUrl = new URL(url).href;
      // Attribuer l'URL validée directement à la propriété src
      image.src = safeUrl;
    } catch (e) {
      // Si l'URL est invalide, la révoquer et lever une erreur
...
export const processImage = async (
  ...
  return new Promise((resolve, reject) => {
    const timeoutId = setTimeout(() => {
      reject(
        new Error("Le traitement a pris trop de temps et a été interrompu"),
      );
    }, 30000);
    ...
    img.onload = async () => {
      try {
        ...
        if (blob.size > opts.maxFileSize * 1024) {
          ...
          reject(
            new Error(
              `L'image est trop volumineuse. Veuillez choisir une image plus légère.`,
            ),
          );
          return;
        }
