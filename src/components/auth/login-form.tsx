...
function LoginForm({ redirectPath = "/dashboard" }: LoginFormProps) {
  ...
  const onSubmit = useCallback(
    async (data: LoginFormData) => {
      ...
      if (isLocked) {
        setError(
          `Trop de tentatives échouées. Réessayez dans ${formattedLockoutTime}.`,
        );
        return;
      }
      ...
      try {
        ...
        if (success) {
          ...
          const getFallbackUserRole = async () => {
            try {
              ...
              if (user?.user_metadata?.role) {
                console.log(
                  "Rôle récupéré via user_metadata:",
              ...
              console.error(
                "Erreur lors de la récupération du rôle alternatif:",
                err,
              );
              return null;
            }
          ...
          try {
            ...
            if (roleError) {
              console.error(
                "Erreur lors de la récupération du rôle via RPC:",
                roleError,
              );
    ...
    async (provider: "google" | "github" | "linkedin") => {
      ...
      try {
        if (provider === "google") {
          ...
          // Utiliser une variable séparée pour le nom du provider OAuth
          const providerName =
    ...
    <div className="w-full max-w-md p-4 sm:p-6 md:p-8 space-y-5 sm:space-y-6 bg-white/30 dark:bg-slate-900/30 rounded-xl shadow-sm shadow-slate-200/50 dark:shadow-vynal-accent-secondary/20 border border-slate-200 dark:border-slate-700/30 transition-all duration-200">
      ...
      <div className="relative">
        ...
        <div className="relative flex justify-center text-[10px] sm:text-xs">
          <span className="px-2 bg-white/30 dark:bg-slate-900/20 text-slate-500 dark:text-vynal-text-secondary">
            Ou par email
          </span>
        </div>
      </div>
      ...
      {error && (
        <div
          className="p-2 sm:p-3 bg-vynal-status-error/10 border border-vynal-status-error/20 rounded-md flex items-start text-vynal-status-error text-xs sm:text-sm"
          role="alert"
        >
      ...
      >
        <div className="space-y-1 sm:space-y-2">
          <Label
            htmlFor="email"
            className="text-xs sm:text-sm text-slate-800 dark:text-vynal-text-primary"
          >
            Email
          </Label>
          <div className="relative">
