...
const ServiceView: React.FC<ServiceViewProps> = (props) => {
  ...
  // Extraction des initiales du freelance pour l'avatar - avec vérification de sécurité
  const freelanceInitials = useMemo(() => {
    try {
      if (!service?.profiles) return "UN";
      const nameSource = service.profiles.full_name || service.profiles.username || 'UN';
      return nameSource
        .split(' ')
        .map(n => n[0])
        .join('')
  ...
  const formattedDescription = useMemo(() => {
    try {
      if (!service?.description) return "Aucune description disponible";
  ...
  const filteredRelatedServices = useMemo(() => {
    ...
    if (!Array.isArray(relatedServices) || relatedServices.length === 0 || !service?.id) {
      return [];
    }
  ...
  try {
                        ...
                        <div className="flex items-center mb-3">
                          <Avatar className="h-12 w-12 mr-3 border border-vynal-purple-mid/30 shadow-sm">
                            <AvatarImage 
                              ...
                              alt={freelance.username || 'Avatar du vendeur'} 
    ...
    console.error("Erreur lors du rendu du ServiceView:", error);
    return (
      <div className="container mx-auto p-4 text-white">
        <Card className="p-4 bg-vynal-purple-dark/90 border-red-500/50">
          <h2 className="text-xl font-bold text-red-500">Erreur lors du chargement</h2>
          <p className="text-vynal-text-secondary mt-2">
            Une erreur est survenue lors de l'affichage du service. Veuillez réessayer.
          </p>
          {onBack && (
