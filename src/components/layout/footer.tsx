    ...
    links: [
      {
        ...
        href: `${PUBLIC_ROUTES.SERVICES}?category=developpement-web-mobile`,
      },
      {
        name: "Design Graphique",
        href: `${PUBLIC_ROUTES.SERVICES}?category=design-graphique`,
      },
      {
        name: "Marketing Digital",
        href: `${PUBLIC_ROUTES.SERVICES}?category=marketing-digital`,
      },
      {
        name: "Rédaction & Traduction",
        href: `${PUBLIC_ROUTES.SERVICES}?category=redaction-traduction`,
      },
      {
        name: "Vidéo & Audio",
        href: `${PUBLIC_ROUTES.SERVICES}?category=video-audio`,
      },
      {
        name: "Informatique & Réseaux",
        href: `${PUBLIC_ROUTES.SERVICES}?category=informatique-reseaux`,
      },
    ...
    links: [
      {
        ...
        href: PUBLIC_ROUTES.TERMS_OF_SERVICE,
      },
      {
...
const ContactInfo = memo(() => {
  ...
  return (
    <div className="flex flex-col md:flex-row md:space-x-8 space-y-4 md:space-y-0">
      ...
      >
        <Mail
          className="w-4 h-4 mr-2 text-gray-400 dark:text-vynal-text-secondary flex-shrink-0"
          strokeWidth={2}
        />
        <a
    ...
    >
      {theme === "dark" ? (
        <>
          ...
          <span className="text-xs font-medium text-vynal-text-secondary group-hover:text-vynal-text-primary">
            Mode clair
          </span>
        </>
      ) : (
        <>
          <Moon
            className="w-3.5 h-3.5 text-vynal-accent-primary"
            strokeWidth={2.5}
          />
  ...
  useEffect(() => {
    const loadCurrencies = async () => {
      try {
        ...
        const mainCurrencies = FOOTER_CURRENCIES.map((code) =>
          data.find((c: any) => c.code === code),
        ).filter(Boolean);
      ...
      toast.success(
        <div className="flex flex-col gap-1">
          ...
          <div className="text-xs opacity-80">
            Tous les prix de l'application ont été convertis.
          </div>
        </div>,
        { duration: 3000 },
      );
  ...
  return (
    <div className="flex items-center gap-2 w-full md:w-auto">
      <div className="text-[10px] text-gray-400 dark:text-vynal-text-secondary flex-shrink-0">
        <Globe
          className="h-3 w-3 inline-block mr-1 opacity-80"
          strokeWidth={2}
        />
        Devise:
      </div>
      ...
      >
        ...
        <SelectContent
          className="min-w-fit text-[10px] bg-gray-900 border-gray-800 text-gray-300"
        ...
        >
          {availableCurrencies.map((c) => (
            <SelectItem
              key={c.code}
              value={c.code}
              className="py-1 text-[10px] text-gray-300 hover:bg-gray-800/80 hover:text-vynal-accent-primary data-[state=checked]:bg-gray-800 data-[state=checked]:text-vynal-accent-primary"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
          ...
          >
            ...
            <p className="mt-4 text-[10px] text-gray-400 dark:text-vynal-text-secondary leading-relaxed">
              La plateforme de mise en relation entre freelances et clients pour
              des projets réussis.
              <br />
