...
interface UseUserResult {
  ...
  isAdmin: boolean;
  canAccess: (
    requiredRoles: Array<"client" | "freelance" | "admin">,
  ) => boolean;
...
const userProfileCache = new Map<
  string,
  { timestamp: number; data: UserProfile }
>();
...
export function useUser(): UseUserResult {
    ...
    if (loading) {
      ...
      loadingTimeoutRef.current = setTimeout(() => {
        if (loadingRef.current) {
          ...
          console.warn(
            "[useUser] Timeout de chargement atteint, sortie forcée de l'état de chargement",
          );
          setLoading(false);
  ...
  const isAdmin = useMemo(
    () => Boolean(profile?.role === "admin" || profile?.is_admin),
    [profile],
  );
    ...
    async (
      ...
      if (metadataRole && !profileRole) {
        try {
          ...
          console.error(
            "Erreur lors de la mise à jour du rôle dans le profil:",
            error,
          );
        }
      }
      ...
      if (profileRole && !metadataRole) {
        try {
          ...
          console.error(
            "Erreur lors de la mise à jour du rôle dans les métadonnées:",
            error,
          );
        }
      }
  ...
  const updateProfile = useCallback(
    async (updates: Partial<UserProfile>): Promise<UpdateProfileResult> => {
      if (!user) {
        ...
        try {
          ...
          if (user.user_metadata?.role) {
            const tempProfile: UserProfile = {
              ...
              verification_level:
                user.user_metadata?.verification_level || null,
              last_seen: null,
              ...
              certified_at: null,
              certification_type: null,
              is_currency_manually_set:
                user.user_metadata?.is_currency_manually_set || false,
              last_currency_rates_update:
                user.user_metadata?.last_currency_rates_update || null,
              custom_currency_rates:
                user.user_metadata?.custom_currency_rates || null,
            };
              ...
              supabase.auth
                ...
                .then(({ data: authData, error }) => {
                  if (!error && authData?.user) {
                    console.log(
                      "Métadonnées d'authentification mises à jour avec succès",
                    );
                  }
                });
            }
  ...
  const canAccess = useCallback(
    (requiredRoles: Array<"client" | "freelance" | "admin">): boolean => {
      // Vérifier d'abord user_metadata pour éviter des requêtes inutiles
      if (
        user?.user_metadata?.role &&
        requiredRoles.includes(user.user_metadata.role as any)
      ) {
        return true;
      }
