...
export function useOrders(options: UseOrdersOptions = {}) {
  ...
  useEffect(() => {
    if (rpcAvailable === null) {
      checkRPCFunctionExists('get_orders_stats').then(available => {
        console.log("[Orders] Fonction RPC get_orders_stats disponible:", available);
  ...
  const fetchStatsViaRPC = useCallback(async (): Promise<OrderStats | null> => {
    ...
    try {
      ...
      console.log(`[Orders] Appel RPC get_orders_stats avec user_id=${profile.id}, role=${role}`);
  ...
  const calculateStatsLocally = useCallback((ordersList: Order[], total: number): OrderStats => {
    return {
      totalCount: total,
      ...
      pendingDelivery: ordersList.filter(o => o.status === 'delivered').length,
      totalValue: ordersList.reduce((sum, order) => 
        sum + (order.total_amount || (order.service?.price || 0)), 0
      )
    };
  }, []);
  ...
  const fetchOrders = useCallback(async (forceRefresh = false) => {
    ...
    if (isFetchingRef.current || (!forceRefresh && NavigationLoadingState.isNavigating)) {
      console.log("[Orders] Requête ignorée: déjà en cours ou navigation en cours");
      return;
    }
    ...
    try {
      console.log(`[Orders] Début de la récupération des commandes (${isFreelance ? 'freelance' : 'client'})`);
      ...
      if (useCache && cacheKey && !forceRefresh) {
        const cachedData = getCachedData<{orders: Order[], total: number}>(cacheKey);
        
        if (cachedData) {
          console.log("[Orders] Données récupérées du cache", cachedData.orders.length);
      ...
      try {
        console.log(`[Orders] Construction de la requête pour tab=${activeTab}, page=${currentPage}, search=${searchQuery || 'none'}`);
        ...
        if (data) {
          console.log(`[Orders] ${data.length} commandes récupérées sur ${count} total`);
          ...
          if (!rpcAvailable || activeTab !== 'all' || searchQuery) {
            if (activeTab === 'all' && !searchQuery) {
              ...
              const localStats = calculateStatsLocally(transformedOrders, count || 0);
              setStats(localStats);
  ...
  }, [profile?.id, isFreelance, activeTab, searchQuery, currentPage, itemsPerPage, cacheKey, useCache, updateLastRefresh, rpcAvailable, fetchStatsViaRPC, statsKey, calculateStatsLocally]);
  ...
  useEffect(() => {
    ...
    console.log("[Orders] Configuration de l'abonnement aux changements en temps réel");
