...
function fetchRevalidate(path: string, forceRevalidate: boolean = false): void {
  ...
  const lastInvalidationTime = typeof window !== 'undefined' && window._pathInvalidationTimestamps 
  ...
  if (!forceRevalidate && (now - lastInvalidationTime) < MIN_CLIENT_REVALIDATION_INTERVAL) {
    console.log(`Invalidation ignorée pour ${path} - trop récente (${Math.floor((now - lastInvalidationTime) / 1000)}s)`);
    return;
  }
  ...
  const timeoutId = setTimeout(() => {
    try {
      ...
      fetch(`/api/revalidate?path=${encodeURIComponent(path)}${forceRevalidate ? '&force=true' : ''}`, {
        method: 'POST',
      ...
      .then(response => {
        if (response.status === 429) {
          ...
          console.log(`Invalidation limitée pour ${path}, pas de nouvelle tentative automatique`);
          return { revalidated: false, throttled: true };
        }
        
        if (!response.ok) {
          console.error(`Échec de l'invalidation via API pour ${path}:`, response.status);
      ...
      .then(data => {
        if (data.revalidated) {
          ...
          console.error(`Erreur lors de l'invalidation de ${path}:`, data.error);
        }
      })
      .catch(err => {
        console.error(`Erreur lors de l'invalidation via API pour ${path}:`, err);
      })
      .finally(() => {
...
export function initStaticPagesInvalidation(): void {
  ...
  if (typeof window !== 'undefined') {
    ...
    console.log('Écouteurs d\'invalidation des pages statiques initialisés (mode client)');
  } else {
    // Côté serveur, utiliser directement revalidatePath
    eventEmitter.on(EVENTS.INVALIDATE_ABOUT, () => revalidatePath(STATIC_PAGES.ABOUT));
    eventEmitter.on(EVENTS.INVALIDATE_HOW_IT_WORKS, () => revalidatePath(STATIC_PAGES.HOW_IT_WORKS));
    eventEmitter.on(EVENTS.INVALIDATE_STATUS, () => revalidatePath(STATIC_PAGES.STATUS));
    eventEmitter.on(EVENTS.INVALIDATE_FREELANCE, () => revalidatePath(STATIC_PAGES.FREELANCE));
    eventEmitter.on(EVENTS.INVALIDATE_CONTACT, () => revalidatePath(STATIC_PAGES.CONTACT));
    eventEmitter.on(EVENTS.INVALIDATE_FAQ, () => revalidatePath(STATIC_PAGES.FAQ));
    eventEmitter.on(EVENTS.INVALIDATE_TERMS, () => revalidatePath(STATIC_PAGES.TERMS));
    eventEmitter.on(EVENTS.INVALIDATE_PRIVACY, () => revalidatePath(STATIC_PAGES.PRIVACY));
    eventEmitter.on(EVENTS.INVALIDATE_CODE_OF_CONDUCT, () => revalidatePath(STATIC_PAGES.CODE_OF_CONDUCT));
    eventEmitter.on(EVENTS.INVALIDATE_HOME, () => revalidatePath(STATIC_PAGES.HOME));
    eventEmitter.on(EVENTS.INVALIDATE_SERVICES, () => revalidatePath(STATIC_PAGES.SERVICES));
    eventEmitter.on(EVENTS.INVALIDATE_STATIC_PAGES, () => {
      Object.values(STATIC_PAGES).forEach(path => revalidatePath(path));
    });
    
    console.log('Écouteurs d\'invalidation des pages statiques initialisés (mode serveur)');
  }
}
...
export function invalidateStaticPage(page: keyof typeof STATIC_PAGES): void {
  ...
  if (typeof window === 'undefined') {
    ...
    try {
      ...
      console.error(`Erreur lors de l'invalidation de ${path} côté serveur:`, error);
    }
  } else {
    // Côté client
...
export function invalidateAllStaticPages(): void {
  if (typeof window === 'undefined') {
    ...
    try {
      ...
      console.error('Erreur lors de l\'invalidation de toutes les pages statiques côté serveur:', error);
    }
  } else {
    // Côté client
    Object.values(STATIC_PAGES).forEach(path => fetchRevalidate(path));
    console.log('Invalidation de toutes les pages statiques demandée côté client');
  }
}
...
export function invalidateServicesPage(): void {
  if (typeof window === 'undefined') {
    ...
    try {
      ...
      console.error('Erreur lors de l\'invalidation de la page des services côté serveur:', error);
    }
  } else {
