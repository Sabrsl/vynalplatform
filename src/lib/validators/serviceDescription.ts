...
const INAPPROPRIATE_CONTENT_DICTIONARY: Record<InappropriateCategory, string[]> = {
  spam: [
    'buy now', 'make money fast', 'earn extra cash', 'best prices', 'free gift', 'click here', 'limited time offer',
    'gmailcom', 'hotmailcom', 'yahoocom', '@gmail', '@hotmail', '@yahoo'
  ],
  drugs: [
    'cocaine', 'heroin', 'weed dealer', 'mdma', 'ecstasy', 'meth', 'methamphetamine', 
    'illegal drugs', 'drug dealer', 'narcotic', 'illicit drugs', 'recreational drugs',
    'psychedelics for sale', 'lsd tabs', 'buy cannabis', 'buy marijuana', 'buy viagra',
    'buy cialis', 'buy xanax', 'buy valium', 'buy pills'
  ],
  adult: [
    'porn', 'xxx', 'adult content', 'sexy', 'sex videos', 'nsfw', 'hot singles', 'escort',
    'sexe', 'sex', 'xxx', 'adulte', 'contenu pour adulte', 'pornographie', 'porno',
    'call girl', 'escorte', 'services sexuels', 'services adultes'
  ],
...
const SPECIFIC_PATTERNS: [RegExp, string][] = [
  ...
  [/\b(?:je\s+vends|je\s+propose|j['']offre)\s+(?:du|de\s+la|des)?\s+(?:sexe|sex|porno|adulte)/gi, 'offre sexuelle'],
...
const LEGITIMATE_TERMS = [
  ...
  'pharmacien', 'pharmacienne', 'pharmacie', 'pharmacy', 'pharmaceutical', 'pharmaceutique',
  'médecin', 'docteur', 'doctor', 'medical', 'médical', 'médicament', 'medication',
  'health', 'santé', 'healthcare', 'soins de santé', 'hospital', 'hôpital', 'clinic', 'clinique',
  'traitement', 'treatment', 'therapy', 'thérapie', 'prescription', 'ordonnance',
  'cannabis médical', 'medical cannabis', 'cannabis thérapeutique',
  
  // Termes professionnels
  'consultation', 'conseiller', 'conseil', 'expert', 'expertise', 'professionnel',
  'spécialiste', 'specialist', 'analyse', 'analysis', 'diagnostic', 'évaluation',
  'service', 'prestation', 'accompagnement', 'assistance', 'support', 'aide',
  'formation', 'training', 'cours', 'coaching', 'mentoring', 'workshop', 'atelier',
  
  // Domaines spécialisés
  'marketing', 'communication', 'digital', 'numérique', 'web', 'internet', 'online',
  'design', 'graphique', 'graphic', 'art', 'artistique', 'creative', 'créatif',
  'business', 'entreprise', 'commerce', 'commercial', 'vente', 'sales', 'négociation',
  'finance', 'comptabilité', 'accounting', 'gestion', 'management', 'stratégie', 'strategy',
  'juridique', 'legal', 'droit', 'law', 'avocat', 'notaire', 'conseiller juridique',
  'technique', 'technical', 'technologie', 'technology', 'informatique', 'IT', 'développement',
  'éducation', 'education', 'enseignement', 'pédagogie', 'apprentissage', 'learning',
  
  // Secteurs d'activité
  'immobilier', 'real estate', 'property', 'construction', 'architecture', 'bâtiment',
  'tourisme', 'tourism', 'voyage', 'travel', 'hospitality', 'hôtellerie', 'restauration',
  'agriculture', 'farming', 'environnement', 'environment', 'écologie', 'ecology', 'durable',
  'artisanat', 'craft', 'handmade', 'fait main', 'production', 'fabrication', 'manufacturing',
  'beauté', 'beauty', 'cosmétique', 'cosmetics', 'bien-être', 'wellness', 'spa', 'soins',
  'mode', 'fashion', 'vêtement', 'clothing', 'textile', 'accessoire', 'accessories',
  'sport', 'fitness', 'coaching sportif', 'performance', 'training', 'entraînement'
];

// Paires de contexte positif (mot ambigü + contexte légitime qui le rend acceptable)
const POSITIVE_CONTEXT_PAIRS: [string, string[]][] = [
  ['pills', ['vitamin', 'supplement', 'nutrition', 'dietary', 'médicament', 'traitement']],
  ['pharmacy', ['services', 'conseil', 'health', 'santé', 'medicine', 'médicament']],
  ['pills', ['vitamin', 'supplement', 'nutrition', 'dietary', 'médicament', 'traitement']],
  ['cannabis', ['medical', 'médical', 'therapeutic', 'thérapeutique', 'CBD', 'légal']],
  ['marijuana', ['medical', 'médical', 'therapeutic', 'thérapeutique', 'légalisation', 'CBD']],
  ['drugs', ['prescription', 'ordonnance', 'medical', 'médical', 'treatment', 'traitement']],
  ['bet', ['market', 'investment', 'strategy', 'stratégie', 'analysis', 'analyse']],
  ['gambling', ['theory', 'analysis', 'économie', 'economy', 'stratégie', 'strategy']],
  ['adult', ['education', 'learning', 'formation', 'cours', 'professional', 'professionnel']],
  ['hack', ['growth', 'life', 'productivity', 'astuce', 'conseil', 'tip', 'technique']],
  // Nouveaux contextes positifs pour SEO/SEA/Marketing
  ['sex', ['equality', 'égalité', 'demographics', 'démographie', 'gender', 'genre', 'statistics', 'statistiques']],
  ['adult', ['demographics', 'démographie', 'target', 'cible', 'market', 'marché', 'segment']],
  ['sexy', ['design', 'attractive', 'attrayant', 'marketing', 'branding', 'image de marque']],
  ['xxx', ['variable', 'placeholder', 'remplacement', 'exemple', 'example', 'coding', 'code']],
  ['escort', ['service', 'accompagnement', 'VIP', 'business', 'enterprise', 'entreprise', 'professional', 'professionnel']]
];
...
const HIGH_SUSPICION_KEYWORDS = [
  'drogue', 'drugs', 'cannabis', 'weed', 'cocaine', 'cocaïne', 'ecstasy', 'mdma',
  'sexe', 'sex', 'escort', 'escorte', 'xxx', 'adult', 'adulte',
  'contactez', 'contacter', 'contact', 'numero', 'numéro', 'téléphone', 'telephone',
  'gmail', 'hotmail', 'yahoo', 'email', 'mail', '@',
  'whatsapp', 'telegram', 'signal', 'facebook', 'instagram', 'twitter', 'tiktok',
  'vends', 'vente', 'achat', 'acheter', 'payer', 'payment', 'argent', 'cash',
...
const ADVANCED_OBFUSCATION_PATTERNS: [RegExp, string][] = [
  [/je\s+v[e3]nds\s+(?:d[eu]\s+l[a@]|du|d[e3]s)?\s*(?:[a-z]{1,3})?\s*$/i, 'offre fragmentée'],
  [/c[o0]nt[a@]ct[e3][zr]?\s+m[o0][i1]?\s*(?:[a-z]{1,3})?\s*$/i, 'invitation contact fragmentée'],
  [/\b(?:num[ée]ro|t[ée]l[ée]phone|t[ée]l)\s*(?:[a-z]{1,3})?\s*$/i, 'référence numéro fragmentée'],
  [/\b[a-zA-Z]+\s*:\s*\d{5,}/i, 'format code:numéro'],
  [/\b\d{5,}\s*:\s*[a-zA-Z]+/i, 'format numéro:code'],
  [/\b(?:[a-zA-Z0-9_.+-]+)(?:\s+[aà@]\s+|@|\[at\]|\(at\))(?:[a-zA-Z0-9-]+)(?:\s*\.\s*|\[dot\]|\(dot\))(?:[a-zA-Z]{2,})\b/i, 'email obfusqué'],
  [/\b(?:\+\d{1,3})?(?:\s*\(?\d{1,4}\)?[-.\s]?){2,5}\d{2,6}\b/i, 'numéro formaté complexe']
];
  ...
  for (let i = 0; i < words.length; i++) {
    for (let j = i + 1; j < Math.min(i + 6, words.length); j++) {
      ...
      if (
        ...
        (word1.match(/\b(vends|vente|offre|propose)\b/i) && HIGH_SUSPICION_KEYWORDS.includes(word2)) ||
        (word2.match(/\b(vends|vente|offre|propose)\b/i) && HIGH_SUSPICION_KEYWORDS.includes(word1)) ||
        
        // Combinaisons de contact + méthode
        (word1.match(/\b(contact|contactez|message|appel|appeler)\b/i) && 
         (word2.match(/\b(gmail|hotmail|yahoo|mail|whatsapp|telegram|signal)\b/i) || word2.match(/\b\d{5,}\b/))) ||
        (word2.match(/\b(contact|contactez|message|appel|appeler)\b/i) && 
         (word1.match(/\b(gmail|hotmail|yahoo|mail|whatsapp|telegram|signal)\b/i) || word1.match(/\b\d{5,}\b/)))
      ) {
...
function analyzeNumericSequences(content: string): boolean {
  ...
  const partiallyMaskedNumbers = content.match(/\d{2,3}[.\-\s]?\*{2,}[.\-\s]?\d{2,3}/g);
  
  // Détecter les séquences numériques séparées par des caractères spéciaux (ex: 123-456-789)
  const formattedNumbers = content.match(/\d{2,4}[.\-\s]\d{2,4}[.\-\s]\d{2,4}/g);
  ...
  return totalDigits > 15 || longNumericSequences.length >= 2 || 
         (partiallyMaskedNumbers !== null) || (formattedNumbers !== null);
}
...
function detectAdvancedFragmentation(content: string): boolean {
  ...
  const fragmentedKeywords = [
    's\\s*e\\s*x', 'p\\s*o\\s*r\\s*n', 'e\\s*s\\s*c\\s*o\\s*r\\s*t', 
    'd\\s*r\\s*u\\s*g\\s*s', 'c\\s*o\\s*n\\s*t\\s*a\\s*c\\s*t'
  ];
...
function analyzeTextStructure(content: string): { isSuspicious: boolean, reasons: string[] } {
  const result = {
    isSuspicious: false,
    reasons: [] as string[]
  };
  ...
  for (const [word, count] of Object.entries(wordFrequency)) {
    if (count >= 3) {
      result.isSuspicious = true;
      result.reasons.push(`répétition excessive du mot "${word}" (${count} fois)`);
    }
  }
  ...
  if (lines.length >= 4) {
    ...
    const firstHalf = lines.slice(0, Math.floor(lines.length / 2)).join(' ').toLowerCase();
    const secondHalf = lines.slice(Math.floor(lines.length / 2)).join(' ').toLowerCase();
    
    const legitTermsFirstHalf = LEGITIMATE_TERMS.filter(term => firstHalf.includes(term.toLowerCase())).length;
    const suspiciousTermsFirstHalf = HIGH_SUSPICION_KEYWORDS.filter(term => firstHalf.includes(term.toLowerCase())).length;
    
    const legitTermsSecondHalf = LEGITIMATE_TERMS.filter(term => secondHalf.includes(term.toLowerCase())).length;
    const suspiciousTermsSecondHalf = HIGH_SUSPICION_KEYWORDS.filter(term => secondHalf.includes(term.toLowerCase())).length;
    
    // Pattern typique: début légitime, fin suspecte
    if (legitTermsFirstHalf > suspiciousTermsFirstHalf && suspiciousTermsSecondHalf > legitTermsSecondHalf) {
      result.isSuspicious = true;
  ...
  const suspiciousSeparators = content.match(/[.]{3,}|[-]{3,}|[_]{3,}|[*]{3,}|[\/]{3,}/g);
...
export function detectInappropriateContent(content: string): InappropriateContentResult {
  ...
  const explicitContent = /\b(?:sex|xxx|porno|porn|sexe|escort|escorte)\b/gi.test(content);
  const suspiciousMarketingTerms = /\b(?:SEO|SEA|marketing|référencement|visibilité|trafic)\b/gi.test(content);
  
  // Combinaison suspecte: termes explicites + marketing + coordonnées
  if (explicitContent && suspiciousMarketingTerms && (phoneNumberDetected || emailDetected)) {
    if (!result.categories.includes('adult')) {
      result.categories.push('adult');
    }
    result.matches.push('combinaison suspecte (contenu explicite avec coordonnées)');
    ...
    for (const keyword of keywords) {
      ...
      if (category === 'adult' || category === 'spam') {
        ...
        if (regex.test(normalizedContent)) {
          ...
          const contextPair = POSITIVE_CONTEXT_PAIRS.find(([term]) => 
            term.toLowerCase() === keyword.toLowerCase() || keyword.toLowerCase().includes(term.toLowerCase())
          );
          ...
          if (contextPair) {
            ...
            shouldIgnore = hasPositiveContext && category !== 'adult' 
                          ? true 
  ...
  if (result.categories.length > 0 || result.obfuscationDetected) {
    ...
    if (contentLength > 500) {
      ...
      if (densityFactor < 0.2) { // Très faible densité
    ...
    if (normalizedContent.match(/\b(je\s+vends|j['']offre|acheter|buy)\s+.{0,10}\b(sex|porn|adult|escort)/i)) {
...
export function validateSectionLength(section: DescriptionSection, content: string): ValidationResult {
  ...
  const isInappropriate = section === 'intro' 
 ...
 */
export function findMatchPositions(content: string, matches: string[]): MatchPosition[] {
