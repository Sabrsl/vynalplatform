...
export type PaymentValidation = CardValidation | PayPalValidation | MobileMoneyValidation;
...
export function validatePaymentData(method: PaymentMethodType, data: PaymentData): string | null {
  ...
  switch (method) {
    case "card": {
      ...
      if (!data.cardNumber || !validation.cardNumber.test(data.cardNumber.replace(/\s/g, ''))) {
    ...
    case "wave": {
      ...
      if (!data.phoneNumber || !validation.phoneNumber.test(data.phoneNumber.replace(/\s/g, ''))) {
...
export function getWithdrawalMethods() {
  return PAYMENT_METHODS
    .filter(method => ["orange-money", "free-money", "wave"].includes(method.id))
    .map(method => ({
      ...method,
      fee: WITHDRAWAL_FEES[method.id as keyof typeof WITHDRAWAL_FEES],
      processing_time: WITHDRAWAL_PROCESSING_TIMES[method.id as keyof typeof WITHDRAWAL_PROCESSING_TIMES]
    }));
} 