...
import { AlertCircle, AlertTriangle, CheckCircle, RefreshCw, Clock, Info, Shield } from "lucide-react";
...
export default function PublicStatusPage() {
  ...
  return (
    <div className="bg-white dark:bg-gray-900 min-h-screen">
      {/* Bannière de statut */}
      <div className={`w-full py-1.5 text-white text-center ${
        getSystemStatus() === 'functional' ? 'bg-green-600 dark:bg-green-700' :
        getSystemStatus() === 'degraded' ? 'bg-yellow-600 dark:bg-yellow-700' :
      ...
      <div className="container max-w-7xl mx-auto px-4 py-4">
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-3 gap-3">
          <div>
            <h1 className="text-lg font-bold text-gray-900 dark:text-white">Statut du Système</h1>
            <p className="text-[10px] text-gray-600 dark:text-gray-400 mt-0.5">
              Dernière mise à jour: {getLastUpdateTime() ? formatDate(new Date(getLastUpdateTime()!).toISOString()) : 'Chargement...'}
            </p>
          </div>

          <div className="flex items-center gap-3">
            <div className="flex items-center gap-1.5">
              <span className="text-[10px] font-medium text-gray-700 dark:text-gray-300">Incidents actifs:</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
          {loading ? (
            Array(6).fill(0).map((_, index) => (
              <Card key={`skeleton-${index}`} className="cursor-pointer hover:shadow-md transition-shadow bg-white dark:bg-gray-800">
        ...
        <SheetContent className="w-full sm:max-w-md overflow-auto bg-white dark:bg-gray-900">
          {selectedFeature && (
            <>
              <SheetHeader className="mb-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <SheetTitle className="text-base text-gray-900 dark:text-white">{selectedFeature.name}</SheetTitle>
                  </div>
              ...
              <div className="mb-4">
                <h3 className="text-[10px] font-medium text-gray-700 dark:text-gray-300 mb-1.5">Informations</h3>
                <div className="bg-gray-50 dark:bg-gray-900/50 rounded-lg p-2.5 text-[10px]">
                  <div className="grid grid-cols-2 gap-1.5">
                    ...
                    <div className="font-medium text-gray-900 dark:text-white">{selectedFeature.component_type}</div>
                    
                    <div className="text-gray-600 dark:text-gray-400">Dernière mise à jour:</div>
                    <div className="font-medium text-gray-900 dark:text-white">{formatDate(selectedFeature.last_updated)}</div>
              ...
              <div className="mb-4">
                <h3 className="text-[10px] font-medium text-gray-700 dark:text-gray-300 mb-1.5">Historique des incidents</h3>
                      ...
                      >
                        <CardHeader className="py-2 px-3">
                          <div className="flex justify-between items-start gap-1.5">
                            <CardTitle className="text-xs text-gray-900 dark:text-white">{incident.title}</CardTitle>
              ...
              {selectedIncident && (
                <div>
                  <h3 className="text-[10px] font-medium text-gray-700 dark:text-gray-300 mb-1.5">Détails de l'incident</h3>
                  <Card className="bg-white dark:bg-gray-800">
                    <CardHeader className="pb-1.5">
                      <CardTitle className="text-xs text-gray-900 dark:text-white">{selectedIncident.title}</CardTitle>
                    ...
                    <CardContent className="pb-1.5">
                      <p className="text-[10px] text-gray-600 dark:text-gray-400">{selectedIncident.description}</p>
                      
                      <div className="mt-2 grid grid-cols-2 gap-1.5 text-[9px] text-gray-600 dark:text-gray-400">
                        <div>Date de début:</div>
                        <div className="font-medium text-gray-900 dark:text-white">{formatDate(selectedIncident.started_at)}</div>
                        
                        {selectedIncident.resolved_at && (
                          <>
                            <div>Date de résolution:</div>
                            <div className="font-medium text-gray-900 dark:text-white">{formatDate(selectedIncident.resolved_at)}</div>
                          </>
                        )}
                      </div>
                    </CardContent>
                    <CardFooter className="flex flex-col items-start pt-0">
                      <h4 className="text-[10px] font-medium text-gray-700 dark:text-gray-300 mb-1.5">Historique des mises à jour</h4>
                            ...
                            <div key={update.id} className="relative pl-4 pb-3">
                              ...
                              <div className="flex items-start gap-2">
                                ...
                                <div>
                                  ...
                                  <p className="mt-0.5 text-[10px] text-gray-600 dark:text-gray-400">{update.message}</p>
                                </div>
                              </div>
                            </div>
                          ))}
      ...
      <div className="bg-white dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700 py-4">
        <div className="container max-w-5xl mx-auto px-4 text-center">
          <p className="text-[10px] text-gray-600 dark:text-gray-400">
            Vous pouvez signaler un problème à notre équipe de support à <a href="mailto:support@vynalplatform.com" className="text-blue-600 dark:text-blue-400 hover:underline">support@vynalplatform.com</a>
          </p>
        </div>
      </div>
    </div>
