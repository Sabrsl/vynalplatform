...
const getEncryptionKeys = () => {
  ...
  if (!key || !iv) {
    throw new Error(
      "Les clés de chiffrement ne sont pas définies dans les variables d'environnement",
    );
  }
...
function simpleEncrypt(data: string): string {
  ...
  try {
    ...
    if (data.length > 1000000) {
      ...
      return btoa(
        JSON.stringify({ data: { truncated: true }, sensitive: false }),
      );
    }
...
function simpleDecrypt(encryptedData: string): string {
  ...
  try {
    ...
    if (
      decoded.includes('"sensitive":true') ||
      decoded.includes('"truncated":true') ||
      decoded.includes('"error":true')
    ) {
      return JSON.stringify({ data: {}, expiry: 0 });
    }
...
export function getCachedData<T>(key: string): T | null {
  ...
  try {
    ...
    if (containsSensitiveInfo(key)) {
      if (process.env.NODE_ENV === "development") {
        console.warn(
          `Tentative de récupération d'une clé sensible "${key}". Cette opération est ignorée.`,
        );
      }
      return null;
    }
...
export function setCachedData<T>(
  ...
  try {
    ...
    if (containsSensitiveInfo(key) || sensitive) {
      if (process.env.NODE_ENV === "development") {
        console.warn(
          `Tentative de mise en cache d'une donnée potentiellement sensible avec la clé "${key}". Opération ignorée.`,
        );
      }
      return;
    }
    ...
    try {
      ...
      if (jsonString.length > 4000000) {
        // Limite prudente à 4MB
        if (process.env.NODE_ENV === "development") {
          console.warn(
            `Données trop volumineuses pour le cache (${Math.round(jsonString.length / 1024)}KB) pour "${key}". Opération ignorée.`,
          );
        }
        return;
      }
      ...
      if (process.env.NODE_ENV === "development") {
        console.warn(
          `Erreur lors de la sérialisation des données pour "${key}"`,
          stringifyError,
        );
      }
    }
...
export async function createServerCryptoService(): Promise<ServerCryptoService | null> {
  ...
  console.warn(
    "Cette fonction est dépréciée. Utilisez createServerCryptoService depuis @/lib/security/server-crypto-service",
  );
  ...
  if (typeof window !== "undefined") {
    if (process.env.NODE_ENV === "development") {
      console.warn(
        "Le service de cryptographie serveur ne peut pas être créé côté client",
      );
    }
    return null;
  }
  ...
  try {
    ...
    const { createServerCryptoService: officialCreateService } = await import(
      "./security/server-crypto-service"
    );
    ...
    if (process.env.NODE_ENV === "development") {
      console.error(
        "Erreur lors de la création du service de cryptographie:",
        error,
      );
    }
